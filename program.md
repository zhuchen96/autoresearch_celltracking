...
**The goal is simple: get the lowest final_val_metric.** The training always runs for a fixed 5-minute wall clock budget, so you don't need to worry about training duration — it's always 5 minutes. Everything is fair game: change the architecture, optimizer, hyperparameters, augmentation, batch size, or model size. The only constraint is that the code runs without crashing and completes within the time budget.
...
Note that the script is configured to always stop after 5 minutes of training time, so depending on the computing platform of this computer the number of completed epochs will vary. You can extract the key metric from the log file:

```
grep "^final_val_metric:" run.log
```
...
```
commit	final_val_metric	memory_gb	status	description
```

1. git commit hash (short, 7 chars)
2. final_val_metric (last completed epoch, e.g. 1.234567) — use 0.000000 for crashes
...
5. Read out the results: `grep "^final_val_metric:\|^peak_vram_mb:" run.log`
6. If the grep output is empty, the run crashed. Run `tail -n 50 run.log` to read the Python stack trace and attempt a fix. If you can't get things to work after more than a few attempts, give up.
7. Record the results in the tsv (NOTE: do not commit the results.tsv file, leave it untracked by git)
8. If final_val_metric improved (lower), you "advance" the branch, keeping the git commit
9. If final_val_metric is equal or worse, you git reset back to where you started

The idea is that you are a completely autonomous researcher trying things out. If they work, keep. If they don't, discard. And you're advancing the branch so that you can iterate. If you feel like you're getting stuck in some way, you can rewind but you should probably do this very very sparingly (if ever).

**Timeout**: Each experiment should take ~5 minutes of training time (+ a few seconds for startup and eval overhead). If a run exceeds 10 minutes total, kill it and treat it as a failure (discard and revert).
...
As an example use case, a user might leave you running while they sleep. If each experiment takes ~5 minutes of training plus overhead, you can run approx 10/hour, for a total of about 80 over the duration of the average human sleep. The user then wakes up to experimental results, all completed by you while they slept!
